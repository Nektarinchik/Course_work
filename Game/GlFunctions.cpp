...
void input(int key, int x, int y)
{
	switch (key) {
	case GLUT_KEY_UP:
		if (board->GetSnake()->GetDirection() != sDirection::DOWN)
			board->GetSnake()->SetDirection(sDirection::UP);
		break;
	case GLUT_KEY_DOWN:
		if (board->GetSnake()->GetDirection() != sDirection::UP)
			board->GetSnake()->SetDirection(sDirection::DOWN);
		break;
	case GLUT_KEY_RIGHT:
		if (board->GetSnake()->GetDirection() != sDirection::LEFT)
			board->GetSnake()->SetDirection(sDirection::RIGHT);
		break;
	case GLUT_KEY_LEFT:
		if (board->GetSnake()->GetDirection() != sDirection::RIGHT)
			board->GetSnake()->SetDirection(sDirection::LEFT);
		break;
	}
}