#include "Brain.h"
#include "CollectionsExtensions.h"

const float Brain::m_mutationChance_ = 0.02f;

Brain::Brain()
{
	m_inputToHidden_ = *Matrix::BestBrainsFromFile(m_hiddenSize_, m_inputSize_ + 1);
	m_hiddenToOutput = *Matrix::BestBrainsFromFile(m_outputSize, m_hiddenSize_ + 1);
}
...
Brain* Brain::BestBrainsFromFile()
{
	return new Brain(Matrix::BestBrainsFromFile(m_hiddenSize_, m_inputSize_ + 1), 
		Matrix::BestBrainsFromFile(m_outputSize, m_hiddenSize_ + 1));
}
